...
def compute(N,filename,method, sup_or_jux=None,diabete='Non',species = 'human',sup_or_multi = 'superficial',inhibition=None,unx = 'N',preg='non', HT='N', HCa='N', hypoCa = 'N', hypoMg = 'N'):
    cw=Vref*60e6
    ...
    for i in range(N):
        cell[i].hypoCa = hypoCa
    
    for i in range(N):
        cell[i].hypoMg = hypoMg
    ...
    if cell[0].segment == 'mTAL':
        ...
        '''
        Related to Ca2+:
        To replicate Edward's Nephron Paper (Regulation of calcium reabsorption along the rat nephron: a modeling study (2014))
        Skip PT, S3, and SDL and initialize TAL with given conditions:
        
        cell[0].conc[15,0] = 3.75
        cell[0].vol[0] = 6/cw
        '''
    ...
    for i in range(N-1):
        ...
        #print("Int [Ca]: {}".format(cell[i+1].conc[15,:]))
        ...
        if method == 'Newton':
            if species == 'human':
                ...
                if cell[0].preg != 'non':
                    sol, fluxvals[i+1].fluxes= Newton_preg.newton_preg_rat(equations.conservation_eqs,x,i,cell[i])
                else:
            ...
            sol, fluxvals[i+1].fluxes = Newton.broyden(equations.conservation_eqs,x,i,cell[i].segment)
        else:
        ...
        if b == 0:
            print(cell[1].conc[15,0])
            print(cell[1].conc[16,0])
            b= b+1
...
#================================OUTPUT IN TO FILE================================        
    ...
    solute = ['Na','K','Cl','HCO3','H2CO3','CO2','HPO4','H2PO4','urea','NH3','NH4','H','HCO2','H2CO2','glu', 'Ca', 'Mg']

...
def read_params(cell,filename,j):
    ...
    # parameter files specify superficial segmental lengths; for some segments, lengths are different for juxtamedullary nephrons, so we overwrite them here...
            ...
            # Coupled transporters:
                ...
                # Hypercalcemia decreases NaPi2 expression
                if cell.HCa == 'Y':
                    if newdLA.solute_id == [0,7] and newdLA.membrane_id == [0,1]:
                        newdLA.perm = (1-0.85) * newdLA.perm
            ...
            # Specific transporters:
                ...
                if cell.type != 'sup' and cell.segment == 'DCT':
                    if cell.sex == 'male':
                        ...
                        if newTransp.type == 'NaMgX':
                         ...
                         #   newTransp.act = newTransp.act * 0.8
                    else:
                        if newTransp.type == 'TRPM6':
                            newTransp.act = newTransp.act * 1.75
                        if newTransp.type == 'NaMgX':
                ...
                if cell.inhib == 'NHE3-50':
                    ...
                    if cell.segment == 'DCT' and newTransp.type == 'NaKATPase':
                        if cell.sex == 'male':
                            newTransp.act = 2 * value / (href * Cref)
                        if cell.sex == 'female':
                            newTransp.act = 3 * value / (href * Cref)
                    
                    if cell.segment in ['CNT', 'CCD'] and newTransp.type == 'NaKATPase':
                        if cell.sex == 'male':
                            newTransp.act = 2 * value / (href * Cref)
                        if cell.sex == 'female':
                            newTransp.act = 3 * value / (href * Cref)
                ...
                if cell.hypoCa == 'Y':
                    if cell.segment == 'DCT' or cell.segment == 'CNT':
                        if newTransp.type == 'TRPV5':
                            newTransp.act = 1.05 * newTransp.act
            ...
            # Actual volume flows:
                ...
                # SNGFR decreases in hypercalcemia
                if cell.HCa == 'Y':
                    if cell.segment == 'PT' and compart_id[tmp[1]] == 0:
                        cell.vol[0] = (1-0.07) * cell.vol[0]
                        cell.vol_init[0] = cell.vol[0]
...
#                print("Wrong id",id)
        ...
        for i in range(NS):
            ...
            cell.sig[i,5,4] = 0.0  
    file.close()
    #print("charge conservation", sum(cell.conc[:, 0] * zval))

...
parser.add_argument('--inhibition',choices=['ACE','SGLT2','NHE3-50','NHE3-80','NKCC2-70','NKCC2-100','NCC-70','NCC-100',
                                            ...
                                            'TRPM6-99', 'NKCC2-ENaC-70', 'NCC-ENaC-70'],default = None,type = str,help = 'any transporter inhibition?')
...
parser.add_argument('--hypoCa',choices=['N','Y'],default = 'N',type = str,help = 'hypocalcemia?')
parser.add_argument('--hypoMg',choices=['N','Y'],default = 'N',type = str,help = 'hypomagnesemia?')
...
if diabete != 'Non':
    ...
    if HT == 'Y':
        ...
        if hypoCa == 'Y':
            file_to_save = preg+'pregnant_'+species[0:3] + '_hypoCa'
        elif hypoMg == 'Y':
            file_to_save = preg+'pregnant_'+species[0:3] + '_hypoMg'
        else:
    ...
    file_to_save = sex + '_' + species[0:3]+'_hypoCa'

elif hypoMg != 'N':
    file_to_save = sex + '_' + species[0:3]+'_hypoMg'
...
def multiprocessing_func(sup_or_jux):
    compute_segment(sup_or_jux, sex, species, sup_or_multi, diabete, inhib, unx, preg, HT, HCa, hypoCa, hypoMg, file_to_save)
...
if __name__ == '__main__':
    ...
    if sex == 'Male':
        ...
        filename = './datafiles/CCDparams_F_' + species[0:3] + '.dat'
    ccd, fluxvals_ccd = compute(NCCD, filename, 'Newton', diabete=diabete, species=species, sup_or_multi=sup_or_multi, inhibition=inhib, unx=unx, preg=preg, HT=HT, HCa=HCa, hypoCa = hypoCa, hypoMg = hypoMg )
    ...
    if sex == 'Male':
        ...
        filename = './datafiles/OMCDparams_F_' + species[0:3] + '.dat'
    omcd, fluxvals_omcd = compute(NOMCD, filename, 'Newton', diabete=diabete, species=species, sup_or_multi=sup_or_multi, inhibition=inhib, unx=unx, preg=preg, HT=HT, HCa=HCa, hypoCa = hypoCa, hypoMg = hypoMg)
    ...
    if sex == 'Male':
        ...
        filename = './datafiles/IMCDparams_F_' + species[0:3] + '.dat'
    imcd, fluxvals_imcd = compute(NIMCD, filename, 'Newton', diabete=diabete, species=species, sup_or_multi=sup_or_multi, inhibition=inhib, unx=unx, preg=preg, HT=HT, HCa=HCa, hypoCa = hypoCa, hypoMg = hypoMg)

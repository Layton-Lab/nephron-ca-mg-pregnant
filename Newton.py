...
def Jac(func,x,k):
    ...
    for i in range(len(x)):
        ...
        for j in range(len(x)):
            ...
            #if i == 47:
             #   print("Newton",j,i,wa1[j], fvec[j], (-wa1[j]+fvec[j]))
...
def newton_rat(func,x,k,cell):
    ...
    if np.isnan(np.linalg.norm(f)):
        if cell.segment == 'CCD' or cell.segment == 'OMCD' or cell.segment == 'IMCD':
            ...
            raise Exception(cell.segment + ' ' + cell.type + ' norm(f) is Nan' + ' at cell number: ' + str(k) + ' iter: ' + str(iter))

    while(np.linalg.norm(f) > 0.0001) and (iter<maxiter+1): #(iter<300)
        if np.linalg.norm(f)>1e14 or np.isnan(np.linalg.norm(f)):
            if cell.segment == 'CCD' or cell.segment == 'OMCD' or cell.segment == 'IMCD':
                raise Exception('Newton solver diverged in ' + cell.segment + ' at cell number: ' + str(k) + ' error ' + str(np.linalg.norm(f)))
            else:
                raise Exception('Newton solver diverged in '+ cell.type + ' ' + cell.segment + ' at cell number: ' + str(k) + ' error ' + str(np.linalg.norm(f)) + ' iter: ' + str(iter))
        elif iter == maxiter:
            if cell.segment == 'CCD' or cell.segment == 'OMCD' or cell.segment == 'IMCD':
        ...
        #print(len(x))
        #print(J.shape)
        #print(np.where(~J.any(axis=1))[0])
        #u, c = np.unique(J, axis=0, return_counts=True)
        #print((c>1).any())
